	...
	: QWidget(parent)
{
	...
	SetIcon(QPixmap(":/Image/company.png"));
	SetTitle("≤‚ ‘±ÍÃ‚");
}

MyDockTitleBar::~MyDockTitleBar()
{
}
void MyDockTitleBar::SetIcon(QPixmap icon) {
	iconPix = icon;
}
void MyDockTitleBar::SetTitle(QString title) {
	mTitle = title;
}
...
void MyDockTitleBar::paintEvent(QPaintEvent*)
{
	...
	height -= 6;
	QRect iconRect = QRect(rect.left() + 5, rect.top() + 3, height, height);
	QRect titleRect = rect.adjusted(iconRect.width()+10,0,0,0);
	//QPixmap tmpIcon = iconPix.scaled(QSize(height, height));
	if(dw->features() & QDockWidget::DockWidgetVerticalTitleBar) {
		QSize s = rect.size();
		s.transpose();
		rect.setSize(s);
	...
	painter.fillRect(rect.left(), rect.top(), rect.width(), rect.height(), QColor(33,150,243));
	...
	painter.drawText(titleRect, Qt::AlignVCenter, mTitle);
